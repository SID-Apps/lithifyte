...
**Connect a model first.** **Lithifyte Pro** — our own model, the default — needs no key; any OpenAI-compatible endpoint, including Ollama or LM Studio on your own machine, stays local. The local option is the one where nothing about your finances leaves your computer — see [security and privacy](/security-and-privacy) for exactly what is sent in each case.
