...
Releases use [`tools/swap-data.mjs`](tools/swap-data.mjs): one shared shell of code, with each user's finance-data block swapped in. The demo is generated by [`tools/make-demo.mjs`](tools/make-demo.mjs) — deterministic, entirely invented data, regenerable from source so you can verify none of it is real. Your personal build (with your transactions embedded) is just `index.html`'s code plus your data block — and it must never be committed; the `.gitignore` tries to protect you from that mistake.
